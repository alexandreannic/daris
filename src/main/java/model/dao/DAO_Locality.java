...
public class DAO_Locality extends DAO<Locality, Long>
{
	 ...
	 * @return
	 */
	public List<Locality> find(String tagName)
	{
		List<Locality> all = em.createQuery("SELECT l FROM Locality l").getResultList();
		
		List<Locality> selected = new ArrayList<Locality>();
		for(Locality l : all){
			if(l.getName().contains(tagName))
				selected.add(l);
		}
		
		return (List<Locality>)selected;
	}
